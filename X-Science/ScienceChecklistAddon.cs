...
//			_logger.Trace( "Made ScienceContext" );
			...
			_logger.Trace(Localizer.Format("#autoLOC_[x]_Science!_064")/*Done Start*/ );
		}
...
//			_logger.Trace( "OnGameSceneSwitch FROM " + Data.from.ToString( ) );
		...
		private void Load( )
		{
			...
			_logger.Trace(Localizer.Format("#autoLOC_[x]_Science!_065")/*Load*/ );
...
//				_logger.Info( "Ui is hidden in this scene" );
			...
			if( _active )
			{
				_logger.Trace(Localizer.Format("#autoLOC_[x]_Science!_066")/*Already loaded.*/ );
				return;
			}
			
			if( HighLogic.CurrentGame.Mode != Game.Modes.CAREER && HighLogic.CurrentGame.Mode != Game.Modes.SCIENCE_SANDBOX )
			{
				_logger.Info(Localizer.Format("#autoLOC_[x]_Science!_067")/*Game type is */ + HighLogic.CurrentGame.Mode + Localizer.Format("#autoLOC_[x]_Science!_068")/*. Deactivating.*/ );
				_active = false;
				return;
			}
			_logger.Info(Localizer.Format("#autoLOC_[x]_Science!_067")/*Game type is */ + HighLogic.CurrentGame.Mode + Localizer.Format("#autoLOC_[x]_Science!_069")/*. Activating.*/ );
			_active = true;
